...
const int row_d = landmark_num * 2;

const int basis_d = 11;
const int total_basis = 50;
const bool fromtxt = true;
...
namespace dlib
{
	...
	namespace impl
	{
		...
		struct regression_tree
		{
			...
			std::vector<matrix<float, 0, 1> > pca_coefs;

			void set_pca_coefs(matrix<float, row_d, basis_d> basis) {
				for (int i = 0; i < leaf_values.size(); i++) {
					matrix<float, 0, 1> leaf_value = leaf_values[i];
					pca_coefs.push_back(trans(basis) * leaf_value);
				}
				//std::cout << pca_coefs[1].size() << std::endl;
			}

			/*matrix<float, 0, 1> convertLeaf2PcaCoefs(matrix<float, 0, 1> leaf) {
				return trans(gbasis) * leaf;
				}*/

			inline const matrix<float, 0, 1>& operator()(
				const std::vector<float>& feature_pixel_values, matrix<float, row_d, basis_d> basis, int *pos = NULL
				) const
				/*!
				requires
				...
				!*/
			{
				...
				if (usePca) {
					return basis * pca_coefs[i - splits.size()];
				}
			...
			friend void serialize(const regression_tree& item, std::ostream& out)
			{
				...
				if (usePca) {
					dlib::serialize(item.pca_coefs, out);
					//std::cout << "serialize pca coefs" << std::endl;
					//std::cout << item.pca_coefs.size() << " " << item.pca_coefs[0].size()  << std::endl;
				}
				else {
					std::cout << "serialize leaf value " << std::endl;
			...
			friend void deserialize(regression_tree& item, std::istream& in)
			{
				...
				if (usePca) {
					if (fromtxt) {
						...
						dlib::deserialize(item.pca_coefs, in);
						//std::cout << "deserialize pca dat" << std::endl;
					}
				}
				else {
					dlib::deserialize(item.leaf_values, in);
	...
	public:
			...
			!*/
		{
			...
			for (unsigned long iter = 0; iter < forests.size(); ++iter)
			{
				...
				for (unsigned long i = 0; i < forests[iter].size(); ++i) {
					int temp;
					current_shape += forests[iter][i](feature_pixel_values, gbasis, &temp);
					posLeafs.push_back(temp);
				}
		...
		friend void serialize(const shape_predictor& item, std::ostream& out)
		{
			...
			if(usePca) {
				std::cout << "serialize gbasis" << std::endl;
				dlib::serialize(item.gbasis, out);
		   }
		}
		friend void deserialize(shape_predictor& item, std::istream& in)
		{
			...
			if (version == 1) {
				if (fromtxt) {
					//char filename[80];
					//sprintf(filename, "D:/data/basis%d.txt", basis_d);
					//sprintf(filename, "D:/data/basis%d.txt", 11);
					//std::cout << filename << std::endl;
					char filename[] = "D:/data/basis_7300.txt";
					std::cout << filename << std::endl;
					setGbasis(item, filename);
					//setGbasis(item, filename);
					usePca = true;
				}
				dlib::deserialize(item.initial_shape, in);
				dlib::deserialize(item.forests, in);
				std::cout << item.forests.size() << " " << item.forests[0].size() << std::endl;
				if (fromtxt) {
					std::cout << "seting pcaf" << std::endl;
					for (int i = 0; i < item.forests.size(); i++) {
						for (int j = 0; j < item.forests[i].size(); j++) {
							item.forests[i][j].set_pca_coefs(item.gbasis);
						}
					}
				}
				...
				if (usePca) {
					char pcaname[80];
					sprintf(pcaname, "D:/data/sp_pca_%d.dat", basis_d);
					std::cout << "pca serialize " << std::endl;
					//serialize("sp_pca.dat") << item;
					serialize(pcaname) << item;
					std::cout << pcaname << std::endl;
				}
			}
			else if (version == 2) {
				usePca = true;
				...
				dlib::deserialize(item.anchor_idx, in);
				dlib::deserialize(item.deltas, in);
				dlib::deserialize(item.gbasis, in);
				std::cout << item.forests.size() << " " << item.forests[0].size() << std::endl;
				std::cout << item.gbasis << std::endl;
			}
		}
		std::vector<int> getLeafs() {
			return posLeafs;
		}

		friend void setGbasis(shape_predictor& item, const char* filename) {
			FILE *fin = fopen(filename, "r");
			for (int j = 0; j < landmark_num * 2; j++) {
				for (int i = 0; i < total_basis; i++) {
					float temp;
					fscanf(fin, "%f ", &temp);
					if(i >= basis_d) continue;
					item.gbasis(j, i) = temp;
				}
			}
			//std::cout << item.gbasis << std::endl;
			fclose(fin);
			std::cout << "gbasis loading" << std::endl;
		}



	private:
		matrix<float, row_d, basis_d> gbasis;
		...
		mutable std::vector<int> posLeafs;
