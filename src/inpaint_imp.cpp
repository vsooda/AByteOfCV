...
public:
    ...
    bool Push(int i, int j, float T) {
        ...
        while (tmp->prev->T>T) tmp = tmp->prev;   //决定以何排序
		//while(tmp->prev->j > j) tmp = tmp->prev;  //直接用坐标进行
        if (tmp!=empty) {
		...
		/*char msg[50];
		sprintf_s(msg, "push i %3d  j %3d  T %12.4e  in %4d\n",i,j,T,in);
      ...
      for (q=0; q<4; q++) {
         ...
         if (CV_MAT_ELEM(*f,uchar,i,j)==INSIDE) {
            /*dist = min4(FastMarching_solve(i-1,j,i,j-1,f,t),
                        ...
                        FastMarching_solve(i-1,j,i,j+1,f,t),
                        FastMarching_solve(i+1,j,i,j+1,f,t));*/
			float temp1 = FastMarching_solve(i-1,j,i,j-1,f,t);
			float temp2 = FastMarching_solve(i+1,j,i,j-1,f,t);
			float temp3 = FastMarching_solve(i-1,j,i,j+1,f,t);
			float temp4 = FastMarching_solve(i+1,j,i,j+1,f,t);
			dist = min4(temp1, temp2, temp3, temp4);
...
//改进版本
static void
icvTeleaInpaintFMM2(const CvMat *f, CvMat *t, CvMat *out, int range, CvPriorityQueueFloat *Heap ) {
   int i = 0, j = 0, ii = 0, jj = 0, k, l, q, color = 0;
   float dist;
            ...
            if (CV_MAT_ELEM(*f,uchar,i,j)==INSIDE) {
               /*dist = min4(FastMarching_solve(i-1,j,i,j-1,f,t),
                ...
                CV_MAT_ELEM(*t,float,i,j) = dist;
                  ...
                  CvPoint2D32f gradI,gradT,r;
				  float Ia[4]={0},Jx[4]={0},Jy[4]={0},s[4]={1.0e-20f, 1.0e-20f, 1.0e-20f, 1.0e-20f},w,dst,lev,dir,sat;
				  ...
				  int i_border = i + range;
                  for (k=i-range; k<=i_border; k++) {
                     int km=k-1+(k==1),kp=k-1-(k==t->rows-2);
					 int j_border = j + range;
					 for (l=j-range; l<= j_border; l++) {
                        int lm=l-1+(l==1),lp=l-1-(l==t->cols-2);
                        if (k>0&&l>0&&k<t->rows-1&&l<t->cols-1) {
                           if ((CV_MAT_ELEM(*f,uchar,k,l)!=INSIDE)&&
                               ((l-j)*(l-j)+(k-i)*(k-i)<=range2)) {
                              r.y = (float)(i-k);
                              r.x = (float)(j-l);

							 //double vl_r = VectorLength(r); 
                             //dst = (float)(1./(vl_r*sqrt(vl_r)));
							 dst = 1./ VectorLength(r);
                              //lev = (float)(1./(1+fabs(CV_MAT_ELEM(*t,float,k,l)-CV_MAT_ELEM(*t,float,i,j))));
							 lev = CV_MAT_ELEM(*t,float,k,l)-CV_MAT_ELEM(*t,float,i,j);
							  //char msg[50];
							  //sprintf(msg, "%04d%04d%04d%04", k, l, i, j);
							 // std::cout << msg << std::endl;
                              ...
                              dir=VectorScalMult(r,gradT);
                              //if (fabs(dir)<=0.01) dir=0.000001f;
							  if(dir <= 0.01 && dir >= -0.01) dir = 0.0000001f;
                              //w = (float)fabs(dst*lev*dir);
							  w = dst * lev * dir;
							  if(w < 0) w = -w;
...
template <typename T>
void setborder(cv::Mat image, T value) 
{
	int i,j;
	for(j=0; j<image->cols; j++) {
		CV_MAT_ELEM(*image,T,0,j) = value;
	}
	for (i=1; i<image->rows-1; i++) {
		CV_MAT_ELEM(*image,type,i,0) = CV_MAT_ELEM(*image,type,i,image->cols-1) = value;
	}
...
void
customCV::cvInpaint( const CvArr* _input_img, const CvArr* _inpaint_mask, CvArr* _output_img,
           double inpaintRange, int flags )
{
    ...
    if( flags == CV_INPAINT_TELEA )
    {
        ...
        icvTeleaInpaintFMM2(mask,t,output_img,range,Heap);
    }
...
void customCV::inpaint(cv::InputArray _src, cv::InputArray _mask, cv::OutputArray _dst,
                  double inpaintRange, int flags )
{
    cv::Mat src = _src.getMat(), mask = _mask.getMat();
	_dst.create( src.size(), src.type() );
    CvMat c_src = src, c_mask = mask, c_dst = _dst.getMat();
    customCV::cvInpaint( &c_src, &c_mask, &c_dst, inpaintRange, flags );
}
