...
// ----------------------------------------------------------------------------------------
    ...
    public:
		...
		std::vector<float> getInitShapeVector(int landmarkNum) {
			std::vector<float> initShapeVector;
			for (int i = 0; i < 2 * landmarkNum; i++) {
				initShapeVector.push_back(initial_shape(i));
			}
			return initShapeVector;
		}
